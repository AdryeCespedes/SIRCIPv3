...
**Huella de los campos descartados** *(ajuste de la implementación)*: FR-029 compara dos líneas del mismo CUIT en *todos* sus campos, incluidas la razón social y la jurisdicción sede, que no se conservan. Comparar solo los 24 bytes del registro dejaría pasar como idénticas dos líneas que difieren únicamente en la razón social. Por eso el temporal usa registros de **32 bytes**: los 24 del registro definitivo más una huella FNV-1a de 64 bits de esos dos campos, calculada sobre su valor y no sobre su forma entrecomillada. La deduplicación compara los 32 bytes, y la compactación escribe solo los 24 del registro, de modo que el archivo publicado no cambia. Una colisión de la huella entre dos líneas del mismo CUIT —del orden de 2⁻⁶⁴— haría aceptar como idénticas dos líneas que difieren solo en un campo descartado. Ningún importe puede verse afectado por eso, porque los campos que intervienen en el cálculo se comparan exactos.
